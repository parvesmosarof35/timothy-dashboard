...
import { BiShield, BiError, BiRefresh, BiLoaderAlt } from "react-icons/bi";
import Swal from "sweetalert2";

const PrivacyPolicy = () => {
  const { data: apiData, isLoading, error, refetch } = useGetPrivacyPolicyQuery();
  const [updatePolicy, { isLoading: isUpdating }] = useUpdatePrivacyPolicyMutation();

  const editorRef = useRef(null);
  const [privacyPolicy, setPrivacyPolicy] = useState("");

  // Initialize privacy policy content when apiData is fetched
  useEffect(() => {
    if (apiData?.data) {
      setPrivacyPolicy(apiData.data);
    }
  }, [apiData]);

  const handleSave = async (e) => {
    e.preventDefault();
    if (!privacyPolicy.trim() || privacyPolicy.replace(/<[^>]*>/g, "").trim().length < 10) {
      Swal.fire({
        title: "Warning!",
        text: "Privacy Policy content must be at least 10 characters.",
        icon: "warning",
    ...
    try {
      await updatePolicy({ content: privacyPolicy }).unwrap();
...
<<<<<<< HEAD
  const getWordCount = () => {
    if (!privacyPolicy) return 0;
    const cleanText = privacyPolicy.replace(/<[^>]*>/g, "").trim();
...
<<<<<<< HEAD
  const joditConfig = {
    readonly: false,
    placeholder: "Start typing the Privacy Policy...",
    height: 450,
    theme: "default",
    uploader: {
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-6">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-white border border-gray-200 rounded-2xl shadow-sm">
            <BiShield className="w-8 h-8 text-[#F2CA50]" />
          </div>
          <div>
            <h1 className="text-gray-800 text-3xl md:text-4xl font-serif">Privacy Policy</h1>
            <p className="text-gray-400 text-xs mt-1">Manage and update your platform's privacy protocols</p>
          </div>
        </div>
      </header>
      ...
      <form onSubmit={handleSave} className="space-y-8">
        <div className="bg-white border border-gray-200 rounded-[30px] p-6 md:p-10 space-y-6 shadow-md">
          ...
          <div className="space-y-3">
            <label className="text-gray-400 text-[10px] font-bold tracking-[2px] uppercase">Privacy Policy Text</label>
